...
def grid_connections(parent,child):
    ...
    if child_str not in connections[parent_str]:
        ...
        connections[parent_str] = child_str
...
def move(grid):
    ...
    #print(len(moves))
       # print(direction)
        ...
        if unique == True:
            ...
            grid_connections(temp_grid,grid) #connection added
            grid = temp_grid
    ...
    if str_value(grid) == '0123':
        print(connections)
        end = time.time()
        print('Time Taken', end - start)

        currentNode = str_value(grid)
        path = []
        while currentNode != grid_key[0]:
            try:
                path.append(connections[currentNode])
        ...
        finalPath.append(str_value(grid))


        print('The Path from', grid_key[0] , 'to', str_value(grid) , 'is' , finalPath)
        quit()
